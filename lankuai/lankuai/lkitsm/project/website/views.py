...
import time
from .models import china_regionalTablet
import json
from datetime import datetime
from workfolw.models import Workflow ,managerApproval
from login.models import User ,UnitName ,Departments
...
def Iestablishpage(request ,pageid):
    #等钱登录人名字所有工单
    username = request.session['username']
    iestablish = WorkList.objects.all().filter(creator = username).order_by("billingTime")

    paginator = Paginator(iestablish, 7)
...
def delaypage(request ,pageid):
    #当前登录用户姓名
    username = request.session['username']
    delay = WorkList.objects.all().filter(creator = username).filter(workState = '跟进').order_by("billingTime")

    paginator = Paginator(delay, 7)
    page = paginator.page(pageid)
...
def allpage(request ,pageid):
    ...
    if not search:
        error_msg = "请输入搜索关键字"
        allWList = WorkList.objects.all().order_by("-billingTime")
        paginator = Paginator(allWList, 7)
        page = paginator.page(pageid)
        return render(request, 'website/allWorkOrder.html', {'title': '全部工单信息' ,"username":username , "allworkorder":page})
    else:
        allWList = WorkList.objects.filter(workState__icontains=search).order_by("-billingTime")
        paginator = Paginator(allWList, 7)
        page = paginator.page(pageid)
...
def writeWorkList(request):
    ...
    # 取出负责人
    try:
        unitN = UnitName.objects.get(unitname=unitName)
        print(unitN.dname ,"这是该单位负责人------------------------")
    #负责人管辖的所有单位
        unitNList = UnitName.objects.filter(dname=unitN.dname)
    except UnitName.DoesNotExist :
        unitNList = ["未分配"]
    ...
    return render(request, 'website/writeWorkList.html', {'nowTime': nowTime, 'unitName': unitName
            , 'username': username, 'workOrderNo': workOrderNo ,'allUser':allUser ,"unitNList":unitNList})
...
def addWriteWorkList(request):
    if request.method == 'POST':
        ...
        if not (billingTime and gongdanhao and unitName and clientName and clientPho and clientAddress and faultClassList \
            and faultClasssList and faultDetailList and incidentClaList and incidentRankList and incidentStateList \
            and incidentTheme and incidentDescribe and manageCourse and creator and workStateList and flowHandler):
        ...
        print("这里是提交成功之后")
        return HttpResponseRedirect('/addwwlok/')



def addwwlok(request):

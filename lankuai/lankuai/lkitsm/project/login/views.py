...
from .models import Departments ,UnitName
...
def index(request):
    ...
    user = User.objects.get(uname=username)
    print(user ,"user成什么样子了------------------------")

    # 这里你需要把该user的部门id打印出来
    print(user.udepname ,"user.udepname-------------------")
    dep = Departments.objects.get(depname=user.udepname)
    print(dep.did ,"新的dip.did----------------------------")
    ...
    print("公告与公告审批--------------------------" ,user.role ,"------------user.role是什么")

    role = Role.objects.get(rrole=user.role)
    if role.roleid == 0:# 工程师
        role = 0
...
def register(request):
    ...
    return render(request, 'login/register.html', {'title': '注册' ,  "username":username,"roleList":roleList , "depList":depList , "unitname":unitname ,"jobList":jobList})

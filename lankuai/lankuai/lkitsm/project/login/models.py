...
class User(models.Model):
    ...
    # Departments 部门
    udepname = models.ForeignKey("Departments", on_delete=models.CASCADE ,null=True ,blank=True)

    # UnitName
    unit = models.ForeignKey("UnitName", on_delete=models.CASCADE ,null=True ,blank=True)#所在单位

    # Role 角色层级 #角色层级 员工0	，	部门主管1		，部门经理2	，总经理3
    role = models.ForeignKey("Role" ,on_delete=models.CASCADE ,null=True ,blank=True)

    # Jobgrade 临时-1 实习0，试用1，正式2	，1星3，2星4，3星5，4星6，5星7
    jobgrade = models.ForeignKey("Jobgrade" ,on_delete=models.CASCADE ,null=True ,blank=True)
    # technicalGrade 技术等级：12345
    technicalgrade = models.ForeignKey("technicalGrade", on_delete=models.CASCADE ,null=True ,blank=True)
...
class UnitName(models.Model):
    ...
    unitname = models.CharField(r'单位名称', max_length=40 ,primary_key=True)  # 单位名称
    isDelete = models.BooleanField(default=False)
...
class Role(models.Model):
    ...
    rrole = models.CharField(u"角色名称" ,max_length=40)
    isDelete = models.BooleanField(default=False)
    def __str__(self):
        return self.rrole  # 返回属性的显示内同
...
class Jobgrade(models.Model):
    ...
    isDelete = models.BooleanField(default=False)
    def __str__(self):
        return self.jjobgrade  # 返回属性的显示内同
